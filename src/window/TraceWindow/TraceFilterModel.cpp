...
bool TraceFilterModel::filterAcceptsRow(int source_row, const QModelIndex & source_parent) const
{
    ...
    QString datastr0 = sourceModel()->data(idx0).toString().toLower();
    QString datastr1 = sourceModel()->data(idx1).toString().toLower();
    QString datastr2 = sourceModel()->data(idx2).toString().toLower();
    QString datastr3 = sourceModel()->data(idx3).toString().toLower();
    QString datastr4 = sourceModel()->data(idx4).toString().toLower();

    //fprintf(stderr, "Data for acceptance is %s\r\n", datastr1.toStdString().c_str());

    if( datastr0.contains(_filterText.toLower()) ||
        datastr1.contains(_filterText.toLower()) ||
        datastr2.contains(_filterText.toLower()) ||
        datastr3.contains(_filterText.toLower()) ||
        datastr4.contains(_filterText.toLower()))
        return true;
    else
        return false;
}
